...
void init_myfs(struct bb_state *bb_data){
    char *server_address[2];
    server_address[0] = "10.0.41.2";
    server_address[1] = "10.0.41.3";
    int server_port = 3000;
    int clientfd[2] = {0};
    clientfd[0] = socket(AF_INET, SOCK_STREAM, 0);
    clientfd[1] = socket(AF_INET, SOCK_STREAM, 0);
    if(clientfd[0] == -1 || clientfd[1] == -1){
        printf("create socket error. \n");
        return;
    }

    bb_data->client_socket[0] = clientfd[0];
    bb_data->client_socket[1] = clientfd[1];

    struct sockaddr_in serveraddr[2];
    serveraddr[0].sin_family = AF_INET;
    serveraddr[0].sin_addr.s_addr = inet_addr(server_address[0]);
    serveraddr[0].sin_port = htons(server_port);
    serveraddr[1].sin_family = AF_INET;
    serveraddr[1].sin_addr.s_addr = inet_addr(server_address[0]);
    serveraddr[1].sin_port = htons(server_port);

    if(connect(clientfd[0], (struct sockaddr *)&serveraddr[0], sizeof(serveraddr[0])) == -1){
        printf("connect0 error. \n");
        return;
    }
    if(connect(clientfd[1], (struct sockaddr *)&serveraddr[1], sizeof(serveraddr[1])) == -1){
        printf("connect1 error. \n");
        return;
    }
    printf("creat connection succ! \n");
    return;
}
